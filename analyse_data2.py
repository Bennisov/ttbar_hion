...
class ChannelSelector:
    ...
    def process_dielectron(self, el_data, jet_pts, met_data, centrality_flag):
    ...
    def process_electronmuon(self, el_data, mu_data, jet_pts, met_data, centrality_flag):
    ...
    def process_dimuon(self, mu_data, jet_pts, met_data, centrality_flag):
    ...
    def process_mu1jet(self, mu_data, jet_pts, met_data, centrality_flag):
    ...
    def process_el1jet(self, el_data, jet_pts, met_data, centrality_flag):
...
def main():
    ...
    for particle, vars in variables.items():
        if particle == 'met':
            for var in vars:
                data[f'met{var}'] = load_data(file_data, tree, f'met{var}')
        elif particle == 'other':
            for var in vars:
                data[var] = load_data(file_data, tree, var)
        else:
            for var in vars:
                key = f'{particle}_{var}'
                data[key] = load_data(file_data, tree, key)
    ...
    for i in range(n_events):
        ...
        jet_pts = data['jetR2_pt_NOSYS'][i]
        cent_flag = centrality_flag[i]
        
        # *** Process all 5 channels in single pass ***
        selector.process_dielectron(el_data, jet_pts, met_data, cent_flag)
        selector.process_electronmuon(el_data, mu_data, jet_pts, met_data, cent_flag)
        selector.process_dimuon(mu_data, jet_pts, met_data, cent_flag)
        selector.process_mu1jet(mu_data, jet_pts, met_data, cent_flag)
        selector.process_el1jet(el_data, jet_pts, met_data, cent_flag)
